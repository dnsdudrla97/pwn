...
int ptrace_get_##r(struct ptrace_context *p, ptrace_x86_register_t *r)	\
{									\
	long ret;							\
									\
	errno = 0;							\
	ret = ptrace(							\
		PTRACE_PEEKUSER,					\
		p->tid,							\
		offsetof(struct ptrace_registers, r),			\
		NULL							\
	);								\
									\
	if (ret == -1 && errno != 0) {					\
...
int ptrace_get_##r(struct ptrace_context *p,				\
...
int ptrace_get_dr##num(struct ptrace_context *p,			\
